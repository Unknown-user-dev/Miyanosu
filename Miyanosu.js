...
const randomStatus = ['Avez-vous appelé, Unknown?', 'Je\'vous protegerais pour toujours ❤', 'Nous\'serons ensemble pour toujours ❤'];
...
class PatreonHandler {
    static async check(request, reply) {
        try {
            if (!Object.keys(request.query).length|| !request.query.id) {
                reply.code(400);
                return 'Aucune chaîne de requête trouvée';
            }
            ...
            if (!guild) {
                reply.code(500);
                return 'Unknown Guilde non trouvée.';
            }
            ...
            reply.code(404);
            return `L'utilisateur ${member.displayName} (${member.id}) a un rôle Patreon mais n'a pas d'étage`;
        } catch (error) {
            console.error(error);
            reply.code(500);
    ...
    static async getPatreons(request, reply) {
        try {
            ...
            if (!guild) {
                reply.code(500);
                return 'Unknown Guild non trouvé.';
            }
...
class SuzutsukiEvents {
    ready() {
        console.log(`[${this.user.username}]: Maintenant en ligne!`);
        this.user.setPresence({ activity: { name: 'Bonjour Unknown ❤' } })
            .catch(console.error);
        let counter = 0;
    ...
    ShardReconnecting(id) {
        console.log(`[Shard ${id}]: `, 'Reconnexion');
    }

    shardReady(id) {
        console.log(`[Shard ${id}]: `, 'Prêt');
    }

    shardResume(id, rep) {
        console.log(`[Shard ${id}]: `, `Reprise de => ${rep} events`);
    }
    ...
    guildMemberUpdate(oldMember, newMember) {
        ...
        if (!channel) return;
        channel.send(`<a:too_hype:480054627820371977> ${newMember.toString()} est devenu un Patreon ! Merci pour votre soutien \\❤`)
            .catch(console.error);
    }
    ...
    message(msg) {
        ...
        let response;
        if (command === 'ping') {
            response = `Unknown, le ping actuel est \`${msg.guild.shard.ping}ms\``;
        }
        if (command === 'stats' ) {
            const { rss, heapUsed } = process.memoryUsage();
            response = new MessageEmbed()
                .setColor('RANDOM')
                .setAuthor('Situation actuelle')
                .setThumbnail(this.user.displayAvatarURL())
                .setDescription('```ml\n' +
                    `Guildes stockées  :: ${this.guilds.cache.size.toLocaleString()}\n` +
                    `Utilisateurs vus     :: ${this.users.cache.size.toLocaleString()}\n` +
                    `Mémoire utilisée    :: ${this.parseMemory(rss)}\n` +
                    `Amas utilisé     :: ${this.parseMemory(heapUsed)}\n` +
                    '```')
                .setTimestamp()
...
Destroyer.login(config.token)
    .then(() =>
        server.listen(config.restport,
            (error, address) => error ? console.error(error) : console.log(`[REST Server]: Écouter à ${address}`)
        )
    )
    .catch(console.error);
